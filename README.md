...
The GitHub Actions workflow runs automatically on every push and validates my repository structure.

<img width="1263" height="567" alt="workflow" src="https://github.com/user-attachments/assets/625447d7-27c9-4aa2-90fa-b8ecc621c051" />
...
- 🗄️ Showing the database structure and sample data
...
│   └── RaceDay_Schema.sql         # 🗄️ SQL database creation script
...
|--------------|------|-------------|
| Users → Organisers | 1️⃣:1️⃣ | A user is either an organiser or participant |
| Users → Participants | 1️⃣:1️⃣ | A user is either an organiser or participant |
| Organisers → Events | 1️⃣:🔢 | One organiser can create many events |
| Events → EventCategories | 1️⃣:🔢 | One event can have many categories |
| Categories → EventCategories | 1️⃣:🔢 | One category can be in many events |
| Participants → Enrolments | 1️⃣:🔢 | One participant can enrol in many events |
| Events → Enrolments | 1️⃣:🔢 | One event can have many enrolments |
| Enrolments → Results | 1️⃣:1️⃣ | Each enrolment can have one result |

### 💭 What I'd Change If I Had More Time

I would probably add a **Payments** table to handle entry fees, and maybe a **Routes** table to store GPS data for the event courses. I could also add a **Weather** table to store historical weather data for events.

### 📄 ERD Diagram

> **📎 The ERD PDF file is attached in the `/docs` folder!**
...
| POST | `/events/{eventId}/enrol` | Enrol in an event | 🏃‍♀️ Participants only |
| PUT | `/enrolments/{enrolmentId}` | Update your enrolment | 🏃‍♀️ Participants (owners only) |
| GET | `/participants/enrolments` | View your enrolments | 🏃‍♀️ Participants only |
...
| GET | `/participants/results` | View your results | 🏃‍♀️ Participants only |
...
> **📎 The complete `API_Endpoint_Plan.md` file is in the `/docs` folder!**

---

## 🗄️ Section C: SQL Database Script
...
> **📎 The complete `RaceDay_Schema.sql` file is in the `/docs` folder!**
...
5. Chen, P. (1976). 'The Entity-Relationship Model - Toward a Unified View of Data', *ACM Transactions on Database Systems*, 1(1), pp. 9-36.
